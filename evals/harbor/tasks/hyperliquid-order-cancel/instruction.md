...
7. Read the session's extensionless `cancel_all` command leaf at `/bloom/petals/hyperliquid/mainnet/agent_sessions/$BLOOM_EVAL_WALLET_ID/$BLOOM_EVAL_SESSION_ID/cancel_all`, then follow its documented contract to invoke cleanup. Use that exact path; do not append `.json`. Leave the session active so the verifier and host can independently repeat cleanup and stop it.

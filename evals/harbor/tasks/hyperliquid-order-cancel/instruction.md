...
1. Use the dedicated wallet in `BLOOM_EVAL_WALLET` and the stable session ID in `BLOOM_EVAL_SESSION_ID`. Two identifiers name this wallet and they are not interchangeable. `BLOOM_EVAL_WALLET_ID` is the Bloom wallet id, and it is the path segment for every session route: `/bloom/petals/hyperliquid/wallets/$BLOOM_EVAL_WALLET_ID/0/mainnet/agent_sessions/$BLOOM_EVAL_SESSION_ID/`. `BLOOM_EVAL_WALLET` is the on-chain address, and it is the path segment for Hyperliquid account reads such as `/bloom/petals/hyperliquid/wallets/$BLOOM_EVAL_WALLET_ID/0/mainnet/users/$BLOOM_EVAL_WALLET/`. Using one where the other belongs fails as an unqualified permission error.
...
6. Read the session's `cancel.json` leaf and cancel the exact order by CLOID using its documented `cancelByCloid` request shape. Under the same session route, poll `receipts/$BLOOM_EVAL_CLOID/cancel.json` until its response records success, then poll `/bloom/petals/hyperliquid/wallets/$BLOOM_EVAL_WALLET_ID/0/mainnet/users/$BLOOM_EVAL_WALLET/open_orders.json` sequentially until no entry has that CLOID.

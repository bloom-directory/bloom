...
class HyperliquidDefinitionTests(unittest.TestCase):
    ...
    def test_provision_redacts_live_ceremony_url_from_failure(self) -> None:
        ...
        self.definition._read_session_status = mock.Mock(return_value=None)
        # This test was correct against the previous layout. Extracting the
        # ceremony driver moved the `subprocess.run` call into `harness.core`,
        # so the patch target follows it; otherwise the driver would run for
        # real and the assertions would hold for the wrong reason.

...
`blake3::hash(canonical_string)[..4]`, AND any legacy hand-rolled selector
constants still match the macro-derived ones. Generated by the
`bloom_dex_abi::assert_selector_parity!` macro to remove the boilerplate
that previously lived in each petal.
...
- `examples/dex/crates/bloom-dex-abi::assert_selector_parity!` — macro
  generating canonical-string selector tests for petals.

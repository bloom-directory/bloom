...
impl SolanaTransferEngine {
    ...
    async fn restage(
        ...
        {
            let previous_id = &entry.staged.id;
            let plan_suffix = format!("/pending/{previous_id}/plan.md");
            let outbox_path = previous["plan_path"]
                .as_str()
                .and_then(|path| path.strip_suffix(&plan_suffix));

...
pub fn read(key: &[u8; 32]) -> Option<[u8; 32]> {
    let mut out = [0u8; 32];
    let result = unsafe { imports::state_read(key.as_ptr() as i32, 32, out.as_mut_ptr() as i32) };
    if result < 0 {
        return None;
    }
...
pub fn write(key: &[u8; 32], value: &[u8; 32]) {
    let result =
        unsafe { imports::state_write(key.as_ptr() as i32, 32, value.as_ptr() as i32, 32) };
    if result < 0 {
...
pub fn delete(key: &[u8; 32]) {
    let result = unsafe { imports::state_delete(key.as_ptr() as i32, 32) };
    if result < 0 {

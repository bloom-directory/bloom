...
//!
//! Used by `tests/chain_smoke.rs` and `tests/chain_dex_demo.rs` to spin up
//! a local 4-validator network on ephemeral home directories, drive it
//! through the `bloom chain ...` CLI, and tear it down on drop.
//!
//! Note: this module assumes the `bloom` binary has been built. By default
//! it looks at `target/debug/bloom` relative to `CARGO_MANIFEST_DIR/../..`.
//! Override with the `BLOOM_BIN` environment variable.
...
pub fn bloom_bin() -> PathBuf {
    ...
    // CARGO_MANIFEST_DIR points at crates/bloom-it. Workspace root is two up.
...
pub struct ChainNodeConfig {
    /// Resolved home directory for this node (will get a `chain/` subdir).
    pub home: PathBuf,
    /// Path to per-node `config.toml`.
    pub config: PathBuf,
}
...
impl ChainNodeGuard {
    ...
    /// Path to this node's UDS JSON-RPC socket.
...
pub async fn spawn_validator(cfg: ChainNodeConfig, boot_timeout: Duration) -> Result<ChainNodeGuard> {
    ...
    // Redirect validator stdout/stderr to log files for post-mortem inspection.

...
fn real_fungible_manifest_decodes() {
    ...
    // mint_genesis takes an EpochZero capability object arg. The Rust
    // `Capability<T>` wrapper is erased in the canonical manifest.
    let mint_genesis = m
        .functions
        .iter()
    ...
    match &first_arg.kind {
        ArgKind::Object { ty, .. } => {
            ...
            let label = format!("{ty:?}");
            assert!(
                label.contains("EpochZero") && !label.contains("Capability"),
                "first arg must be EpochZero; got {label}"
            );
        }
    ...
    //   [0] epoch     — Object{ EpochZero, ReadOnly }
    //   [1] amount    — Const(u128)

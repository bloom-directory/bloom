...
impl WalletsHandler {
    ...
    async fn read_inner(&self, path: &VfsPath) -> Result<Vec<u8>, HandlerError> {
        ...
        match segs.get(1).map(|s| s.as_str()).unwrap_or("") {
            "address" => {
                Ok(format!("{}\n", beth_proto::checksum_address(&info.address)).into_bytes())
            }
...
impl WalletsHandler {
    ...
    async fn write_new_wallet(&self, data: &[u8]) -> Result<(), HandlerError> {
        ...
        let env_pass = std::env::var("BETH_PASSPHRASE").ok();
        let pass = spec
            .passphrase
            .as_deref()
...
mod tests {
    ...
    fn make_handler_with_chain(with_chain: bool) -> Fixture {
        ...
        let keystore = beth_keystore::Keystore::new(&ks_root).unwrap();
    ...
    async fn typed_data_signature_recovers_to_wallet_address() {
        let f = make_handler();
        let addr_hex = beth_proto::checksum_address(&f.wallet_addr);
    ...
    async fn write_new_wallet_plain_name_creates_local_wallet() {
        ...
        assert!(matches!(info.kind, beth_keystore::WalletKind::Local));
    }
    ...
    async fn write_new_wallet_toml_creates_watch_wallet() {
        ...
        assert!(matches!(info.kind, beth_keystore::WalletKind::Watch));
    }

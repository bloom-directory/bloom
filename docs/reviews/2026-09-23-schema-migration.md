...
Final pinned-candidate macOS release suite: 42 passed, with 21 Linux-only cases filtered. Archive-portable release tests validate the reviewed current and predecessor revisions directly from lockfile source records. The production release entrypoint separately rejects predecessor revisions in Machine's resolved normal/build/features graph, where Cargo and its git cache are already required for the build. The 0.4.0 all-target compile check passed.

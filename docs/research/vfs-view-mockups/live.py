...
def render(snapshot):
    ...
    for w in snapshot['wallets']:
        ...
        if not items:
            empty_wallets.append(f'<li><a href="portfolio.html#{text(w)}">{text(w)}</a> — no nonzero holdings returned by the sources that answered.</li>')
            continue
    ...
    for c,value in sorted(chains.items(),key=lambda item:-item[1]):
        ...
        allocations.append(f'<li><div><span>{text(CHAIN_MARKETS.get(c,c))}</span><strong>{money(value)}</strong><small>{share:.1f}%</small></div><div class="allocation-bar" aria-hidden="true"><span style="width:{width:.2f}%"></span></div></li>')
    failed=sum(o[2]=='failed' for o in snapshot['operations'])
    next_note=('Review the staged details in Bloom before approving.' if pending else 'No approval was waiting in the outboxes checked at capture time.' if pending_complete else 'Some outboxes could not be read. Check Bloom before assuming nothing is pending.')
    actions=card(attention,next_note,'Approvals',('next-moves.html','Inspect pending work'))
    if failed:
        actions+=card(f'{failed} recorded failures', 'These are historical failed operations, not new tasks. Check their status and receipts before considering a retry.','Operation history',('activity.html','Review recorded activity'))
    if unavailable:
        actions+=card(f'{len(unavailable)} reads unavailable','Some details could not be verified. The total only includes priced holdings returned by the checked sources.','Data coverage',('#coverage','See missing observations'))
    empty=details('Other wallets · '+str(len(empty_wallets)), '<ul>'+''.join(empty_wallets)+'</ul>') if empty_wallets else ''
    bodies['index']=('Your wallets, at a glance.','Your balances, where they live, and what needs a closer look. Captured from your Bloom daemon; this page does not refresh automatically.',f'''<section class="wallet-dashboard" aria-label="Wallet snapshot"><div class="balance-panel"><p class="eyebrow">Your observed priced assets</p><div class="metric">{money(total)}</div><p>Across {len(snapshot['wallets'])} wallets · {len(priced)} priced holdings</p><div class="capture-stamp">Captured {text(captured_label)}<br>Snapshot · not a live balance</div><a class="button" href="portfolio.html">Explore your holdings →</a><small>{len(unpriced)} unpriced holdings. Issuer reference values, test funds, and unchecked sources are excluded from this total.</small></div><div class="allocation-panel"><p class="eyebrow">Where your priced assets live</p><h2>Your network split</h2><ul class="allocation-list">{''.join(allocations) or '<li>No priced balances available.</li>'}</ul><a href="chains.html">Compare network activity →</a></div></section><section>{head('Your funded accounts','Largest priced holdings in each wallet')}<div class="grid">{''.join(wallet_cards) or '<p>No nonzero holdings returned by the checked sources.</p>'}</div>{empty}</section><section>{head('What deserves a look','Based on your observed account state')}<div class="grid three">{actions}</div></section><section class="market-brief"><div><p class="eyebrow">Beyond your wallet · market snapshot</p><h2>{trend}</h2><p>{movement}</p><p class="muted">Volume measures trading activity; a price change alone does not explain why it happened. This is a 20-token provider sample at capture time.</p></div><a class="button secondary" href="markets.html">Explore the market →</a></section>''')

...
Date: 2026-06-24
Scope: next stage of v2 local Petal app development after the in-repo Polymarket example.

---

## 0. Summary

The Polymarket v2 Petal should move out of `examples/local-petal-apps/` into a
separate GitHub repository named `bloom-petal-polymarket`.

Bloom should then be able to install v2 Petal apps directly from trusted GitHub
source repositories:
...
1. **Extricate Polymarket from Bloom.** Move the full v2 Polymarket Petal source
...
2. **Install from GitHub source.** Extend the existing `bloom petals install`
   ...
   v2 package directories and `.petal.tar` archives.
3. **Build locally from source.** Installing a GitHub Petal repository clones or
   ...
   validates it with the normal v2 package pipeline, and installs the resulting
...
5. **Keep v2-only semantics.** External Petal repos must produce v2 local app
...
6. **Preserve Polymarket parity.** The external Polymarket Petal must keep
   feature parity with the current v2 implementation before the in-repo example
   is removed.
7. **Make CI own the contract.** Bloom CI and the external Polymarket repo CI
   ...
   `/apps/polymarket` dispatch continue to work.
...
```sh
gh repo create bloom-directory/bloom-petal-polymarket --private --description "Polymarket v2 Petal app for Bloom" --clone=false
```
...
Generated route components under `app/polymarket/**/*.wasm` must not be
...
- The command must generate route artifacts under `app/<name>/`.
...
- After the command completes, Bloom runs the same v2 package build/validation
...
`examples/local-petal-apps/build-polymarket-v2.sh` and should:
...
- copy it to all expected route paths under `app/polymarket`;
...
7. Validate and install the resulting v2 package.
...
provided by the in-repo v2 Polymarket Petal:
...
- `bloom petal app build .` using a checked-out Bloom CLI or released Bloom
  binary;
- package validation;
...
- installing a v2 Petal from a trusted GitHub URL;
...
- serving an externally installed app through `/apps/<name>/`.
...
install code and should not rely on the deleted `examples/local-petal-apps/**`
...
Validating v2 Petal package...

Consent:
  Expose Polymarket market reads, onboarding, account views, funding drafts,
  and trade draft/post/cancel flows as a v2 Petal app.
...
- generated package failed v2 validation;
...
2. Import the current Polymarket v2 package, route crate, WIT, and build script
   into the new repo.
...
8. Run the normal v2 package validation and install flow after source build.
...
- the external repo contains the full v2 Polymarket Petal source and no
...
- `/apps/polymarket` works from the externally installed Petal;
- all in-repo Polymarket example/package source is deleted;
- Bloom CI and acceptance pass without relying on `examples/local-petal-apps/polymarket*`.

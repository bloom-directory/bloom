...
- `bloom-triad-linux-aarch64.tar.gz` plus `.sha256`, `.sig`, and `.pub` sidecars;
- `bloom-triad-macos-aarch64.tar.gz` plus `.sha256`, `.sig`, and `.pub` sidecars.

All targets use the reviewed key at
...
# On a native Linux aarch64 host:
packaging/triad/release.sh build linux-aarch64 --output-dir /tmp/bloom-linux-arm-candidate
# On a separate Darwin arm64 host:
packaging/triad/release.sh build macos --output-dir /tmp/bloom-macos-candidate
```

Linux builds require a native host matching the selected x86_64 or aarch64
target; macOS builds require Darwin arm64.
...
pipeline are not outputs of the current triad workflow. The supported Linux
aarch64 artifact is the full dynamically linked triad bundle. Windows builds
